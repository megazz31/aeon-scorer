...
import { FriendlyCopyObserver } from './UxEnhancements.jsx'
...
export function CompareDecksPage(){return <div className="uxRouteShell uxCompareRoute"><FriendlyCopyObserver/><RouteIntro kind="compare"/><PodMatchPage/></div>}
export function EventTablesPage(){
  ...
  return <div className="uxRouteShell uxTablesRoute"><FriendlyCopyObserver/><RouteIntro kind="tables"/><AeonMatchPage/></div>
}
export function SharePage(){
  const lang=language(),code=codeOf(location.pathname)
  const seedTables=()=>sessionStorage.setItem('aeon-seed-share',code)
  return <div className="uxRouteShell uxShareRoute"><FriendlyCopyObserver/><SharedAnalysisPage/><div className="shareWorkflowBar"><b>{t('Use this snapshot','Utiliser ce snapshot')}</b><a className="uxPrimaryAction" href={`/pod?d=${code}`}>{productLabel('compare',lang)}</a><a href={`/match?d=${code}`} onClick={seedTables}>{productLabel('tables',lang)}</a><a href={`/tournoi?d=${code}`}>{productLabel('tournament',lang)}</a></div></div>
}
