...
export function Rule0Card({share,compact=false}){const d=normalizedShare(share),signals=d.bracketSignals||{},cmd=d.commanderNames.join(' + ')||d.deckName,pi=d.productIntelligence||{},spof=Object.entries(pi.spof?.dependencies||{}).sort((a,b)=>(b[1]?.score||0)-(a[1]?.score||0))[0];return <article className={`rule0Card${compact?' compact':''}`}>
  ...
  <Range d={d}/>
  <div className="rule0Signals"><span>{d.gameChangers.length} Game Changer{d.gameChangers.length===1?'':'s'}</span>{signals.spellbookBracket&&<span>Spellbook · {signals.spellbookBracket}</span>}{Number(signals.twoCardCombos)>0&&<span>{signals.twoCardCombos} {t('two-card combo(s)','combo(s) 2 cartes')}</span>}{spof&&<span>SPOF · {pretty(spof[0])} {spof[1].level}</span>}</div>
  {!!d.gameChangers.length&&<small className="rule0Gc">{d.gameChangers.join(' · ')}</small>}
  <footer>Aeon {d.engineVersion} · {d.semanticVersion} · {d.iterations.toLocaleString()} {t('sequences','séquences')}{pi.modelVersion?' · Intelligence experimental':''}</footer>
</article>}
