...
export function validateGameObservation(input={}){
  const errors=[],turnBand=String(field(input,'turnBand','turn_band')||''),winType=field(input,'winType','win_type'),balance=input.balance,dominantEvent=field(input,'dominantEvent','dominant_event')||'none',podModelVersion=String(field(input,'podModelVersion','pod_model_version')||'').trim(),podSize=num(field(input,'podSize','pod_size')),predictedRiskScore=num(field(input,'predictedRiskScore','predicted_risk_score')),predictedRiskLevel=String(field(input,'predictedRiskLevel','predicted_risk_level')||''),predictedPodMismatch=num(field(input,'predictedPodMismatch','predicted_pod_mismatch')),predictedThreatGap=num(field(input,'predictedThreatGap','predicted_threat_gap'))
  ...
  if(!(podSize>=2&&podSize<=8&&Number.isInteger(podSize)))errors.push('invalid-pod-size')
  ...
  const expectedRiskLevel=predictedRiskScore>=70?'high':predictedRiskScore>=40?'moderate':'low';if(allowedRisk.has(predictedRiskLevel)&&predictedRiskLevel!==expectedRiskLevel)errors.push('inconsistent-predicted-risk-level')
...
export function summarizeRealityObservations(observations=[]){
  ...
  return {modelVersion:'aeon-reality-v1',count:valid.length,distinctPlaygroups:groups.size,distinctPods:pods.size,balanceIndex:Math.round(avg),severeMismatchRate:valid.length?Number((severe.length/valid.length).toFixed(3)):0,normalGameRate:valid.length?Number((normal.length/valid.length).toFixed(3)):0,meanPredictedRisk:valid.length?Number((valid.reduce((s,x)=>s+x.predictedRiskScore,0)/valid.length).toFixed(2)):0,byPodSize:Object.fromEntries([...new Set(valid.map(x=>x.podSize))].sort((a,b)=>a-b).map(k=>[k,valid.filter(x=>x.podSize===k).length])),byRiskLevel:Object.fromEntries([...allowedRisk].map(k=>[k,valid.filter(x=>x.predictedRiskLevel===k).length])),byDominantEvent:Object.fromEntries([...allowedEvent].map(k=>[k,valid.filter(x=>x.dominantEvent===k).length])),confidence:{productCalibration:'observational-only'}}
}
...
export function calibrationReadiness(observations=[],options={}){
  ...
  return {modelVersion:'calibration-readiness-v2',ready,summary,evaluation,requirements:{minGames,minDistinctPods,holdoutRequired:true,baselineComparisonRequired:true,calibrationCurveRequired:true,podSizeCohortReviewRequired:true,podLeakageProtection:true,playgroupLeakageProtectionBeforePublicProbability:true},reasons,notes:['Readiness is necessary, not sufficient, for publishing exact game-quality probabilities.','A true production promotion still requires a held-out cohort, pod-size cohort review and protection against playgroup leakage.','No observation automatically rewrites card semantics or deck power truth.']}
}
