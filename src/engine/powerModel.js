...
export function analyzePower(rawCards,rawCommander=null,aeonMap=null,iterations=3000){
  ...
  const aeon=aeonSignal(cards.concat(commander?[commander]:[]),aeonMap),packageCohesion=weightedPackageCohesion(packages),comboBoost=combos.reduce((s,c)=>s+c.severity*14,0),t3=sim.turnProfile.find(x=>x.turn===3)||{},t4=sim.turnProfile.find(x=>x.turn===4)||{}
  ...
  const floor=calibratePower(clamp(sim.floor+shift)),median=calibratePower(structural),ceiling=calibratePower(clamp(sim.high+shift)),peak=calibratePower(clamp(sim.peak+shift+comboBoost*.18)),dispersion=Math.max(0,ceiling-floor),commanderDelta=commander&&simNoCmd?Math.max(0,Math.round((sim.median-simNoCmd.median)+cmdSyn.score*.08)):0
