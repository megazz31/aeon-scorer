...
function hasPerTargetCommanderDiscount(c){return /spells you cast cost \{1\} less to cast for each target/i.test(String(c?.oracle||''))}
...
export function analyzePower(rawCards,rawCommander=null,aeonMap=null,iterations=3000,options={}){
  ...
  const dimensions={speed:Math.round(speed),consistency:Math.round(consistency),explosiveness:Math.round(explosiveness),synergy:Math.round(synergy),interaction:Math.round(interaction),resilience:Math.round(resilience)},warnings=[],targetDiscountNotSimulated=commanders.some(hasPerTargetCommanderDiscount)
  ...
  if(targetDiscountNotSimulated)warnings.push('Réduction de coût du commandant « par cible » détectée : cette réduction n’est pas encore appliquée aux courbes de lancement. Le score peut donc être conservateur pour les decks de type Hinata.')
