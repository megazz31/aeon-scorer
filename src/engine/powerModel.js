...
export function analyzePower(rawCards,rawCommander=null,aeonMap=null,iterations=3000,options={}){
  ...
  const dimensions={speed:Math.round(speed),consistency:Math.round(consistency),explosiveness:Math.round(explosiveness),synergy:Math.round(synergy),interaction:Math.round(interaction),resilience:Math.round(resilience)},warnings=[],targetDiscountNotSimulated=commanders.some(hasTargetCommanderDiscount),topLibraryCheatNotSimulated=commanders.some(hasTopLibraryCreatureCheat)
  ...
  if(targetDiscountNotSimulated)warnings.push('Réduction de coût du commandant basée sur les cibles détectée : la relation structurelle est reconnue, mais la réduction n’est pas encore appliquée aux courbes de lancement. Le score peut être conservateur pour des commandants comme Hinata ou Killian.')
  if(topLibraryCheatNotSimulated)warnings.push('Mise en jeu gratuite depuis le dessus de la bibliothèque détectée sur le commandant : la densité de cibles et le mana économisé ne sont pas encore simulés. Le score peut être conservateur pour des commandants comme Ureni.')
