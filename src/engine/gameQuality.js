...
export function buildGameQualityForecast(results=[],podMatch=null,threatTimeline=null){
  ...
  risk=Math.round(clamp(risk));const answerTimingV2=['threat-answer-v3','threat-answer-v4'].includes(threat?.modelVersion)||match?.modelVersion==='advanced-pod-match-v4'
  return {modelVersion:answerTimingV2?'game-quality-v3':'game-quality-v2',risk:{score:risk,level:level(risk)},compatibility:risk<40?'good':risk<70?'mixed':'poor',reasons,confidence:{productCalibration:'experimental',answerTiming:answerTimingV2?'class-specific-v2':'legacy'},notes:[answerTimingV2?'V3 combines multi-axis pod mismatch with class-specific Threat–Answer exposure; Threat–Answer V4 preserves the same worst-window arithmetic while retaining all threat windows.':'V2 combines multi-axis pod mismatch, Threat–Answer exposure and explicit vulnerability-versus-answer hard-counter signals.','This remains a categorical risk model, not a win-rate or exact good-game probability.','Agency Timeline V1 is diagnostic only and is intentionally not included in this risk score.','Politics, pilot skill and unmodeled card text remain outside this forecast.']}
}
