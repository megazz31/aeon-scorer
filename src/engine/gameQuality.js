...
export function buildGameQualityForecast(results=[],podMatch=null,threatTimeline=null){
  ...
  risk=Math.round(clamp(risk));const answerTimingV2=threat?.modelVersion==='threat-answer-v3'||match?.modelVersion==='advanced-pod-match-v4'
  return {modelVersion:answerTimingV2?'game-quality-v3':'game-quality-v2',risk:{score:risk,level:level(risk)},compatibility:risk<40?'good':risk<70?'mixed':'poor',reasons,confidence:{productCalibration:'experimental',answerTiming:answerTimingV2?'class-specific-v2':'legacy'},notes:[answerTimingV2?'V3 combines multi-axis pod mismatch with Threat–Answer V3 exposure driven by class-specific Answer Timing V2.':'V2 combines multi-axis pod mismatch, Threat–Answer exposure and explicit vulnerability-versus-answer hard-counter signals.','This remains a categorical risk model, not a win-rate or exact good-game probability.','Politics, pilot skill and unmodeled card text remain outside this forecast.']}
}
