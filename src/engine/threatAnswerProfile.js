...
export function buildThreatProfile(result={},cards=[]){
  const built=buildThreatObjects(result),threats=built.objects,structuredCombo=threats.some(x=>x.id==='combo'&&x.executionPrerequisites?.modelVersion==='combo-execution-eligibility-v1')
  return {modelVersion:'threat-profile-v4',threatObjectModel:built.modelVersion,threats,objects:threats,confidence:{classification:'aggregate-structural-evidence',timing:built.confidence.timing,prerequisites:structuredCombo?'structured-execution-v1':'explicit-partial',productCalibration:'experimental'},notes:['Threat Profile V4 is backed by Threat Objects V2 and preserves the existing threat strength/timing surfaces while adding structured combo execution-prerequisite evidence.','Combo prerequisite state describes Aeon evidence coverage, not proof that a real-game board currently satisfies the requirement.','Unknown or unsupported required prerequisites continue to block exact combo execution timing; no execution/win probability is promoted.','Threat objects remain disruption-needs evidence, not deterministic win conditions.']}
}
