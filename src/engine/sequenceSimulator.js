...
function paymentResult(card,sources,tax=0){for(const pool of expandManaPools(sources)){const used=concretePaymentIndices(card,pool,tax);if(used)return {pool,used}}return null}
export function canPay(card,sources,tax=0){return paymentResult(card,sources,tax)!==null}
function payAndRemain(card,sources,tax=0){const result=paymentResult(card,sources,tax);return result?result.pool.filter((_,i)=>!result.used.has(i)):null}
