...
function isManaSourceText(o){const s=withoutReminderText(o).toLowerCase();return clauses(s).some(c=>{if(/(?:target|each|an?) opponents? [^.]{0,100}(?:adds?|creates? [^.]*treasure)|(?:its|that|their) controller creates? [^.]*treasure|that player creates? [^.]*treasure/.test(c)&&!/\byou [^.]{0,60}(?:add|create)/.test(c))return false;return /(?:^|[:;,—-]\s*)add (?:\{|one mana|two mana|three mana|four mana|x mana)|\byou (?:add|create) [^.]{0,100}treasure|(?:^|,|\bthen\b)\s*create [^.]{0,100}treasure token/.test(c)})}
