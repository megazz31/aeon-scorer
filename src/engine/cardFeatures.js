...
function lifeGainSource(o){const s=withoutReminderText(o).toLowerCase();if(/\blifelink\b/.test(s))return true;return clauses(s).some(c=>{const actionable=c.replace(/\b(?:whenever you (?:would )?gain life|if you gained life[^,]*)(?:,\s*|$)/,'');if(/opponents? gains? [^.]*life/.test(actionable)&&!/\byou (?:may )?gain [^.]*life/.test(actionable))return false;return /\byou (?:may )?gain [^.]*life\b/.test(actionable)||/\beach player gains? [^.]*life\b/.test(actionable)||/\btarget player gains? [^.]*life\b/.test(actionable)})}
