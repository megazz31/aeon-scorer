...
function isTargetRemoval(o){const s=withoutReminderText(o).toLowerCase(),temporary=isTemporaryInteraction(s);if(abilities(s).some(a=>/choose target (?:artifact|creature|enchantment|planeswalker|permanent|nonland permanent)[^.]*\.\s*(?:its|that permanent's|that creature's) owner shuffles? (?:it|that [^.]{0,40}) into [^.]{0,50}\blibrary\b/.test(a)))return true;for(const c of clauses(s)){if(isOwnTargetClause(c))continue;if(/destroy target /.test(c))return true;if(/exile (?:up to )?(?:one other |one |another )?target /.test(c)&&!temporary&&!/return .*battlefield/.test(c))return true;if(/return target [^.]+ to (?:its|their|that player's|owner'?s) hand/.test(c))return true;if(/target [^.]+ gets -(?:\d+|x)\/-(?:\d+|x)/.test(c))return true;const libraryObject=/(?:target (?:attacking |blocking )?(?:artifact|creature|enchantment|planeswalker|permanent|nonland permanent|historic permanent|spell)|target [^.]{0,60}\b(?:artifact|creature|enchantment|planeswalker|permanent|spell)\b)/.test(c);if(libraryObject&&(/put target [^.]{0,120}(?:into|on) [^.]{0,80}\blibrary\b/.test(c)||/target [^.]{0,100}owner shuffles? (?:it|that [^.]{0,40}) into [^.]{0,50}\blibrary\b/.test(c)||/shuffle target [^.]{0,100} into [^.]{0,50}\blibrary\b/.test(c)||/(?:owner|controller) of target [^.]{0,100} shuffles? (?:it|that [^.]{0,40}) into [^.]{0,50}\blibrary\b/.test(c)))return true}return false}
...
const COUNTER_KEYWORD=/\b(?:evolve|adapt|amass|support|bolster|graft|modular|outlast|renown|backup|monstrosity|explores?|connives?|blights?|fabricate|riot|bloodthirst|undying|persist|unleash|scavenge|training|mentor|incubate|devour)\b/
...
function tokenRoles(o){const s=o.toLowerCase(),cs=clauses(s),producer=/create [^.]*\btokens?\b/.test(s)||cs.some(investigatesForYou)||/\b(?:amass|incubate|fabricate)\b/.test(s),payoff=/\btokens?\b you control|creature \btokens?\b (?:you control )?(?:get|have)|whenever [^.]*\btokens?\b[^.]*enters|whenever one or more \btokens?\b|whenever you create [^.]*\btokens?\b|sacrifice (?:a|one or more) \btokens?\b|\bif you created a token\b|\bnumber of tokens you created\b|\bfor each token you control\b|\bfor each creature token you control\b|\bdestroy all nontoken creatures\b/.test(s),doubler=/if [^.]*would create [^.]*\btokens?\b[^.]*twice|twice that many [^.]*\btokens?\b|create twice that many [^.]*\btokens?\b/.test(s);return {producer,payoff,doubler}}
...
function isManaSourceText(o){const s=withoutReminderText(o).toLowerCase();return clauses(s).some(c=>{if(/(?:target|each|an?) opponents? [^.]{0,100}(?:adds?|creates? [^.]*treasure)|(?:its|that|their) controller creates? [^.]*treasure|that player creates? [^.]*treasure/.test(c)&&!/\byou [^.]{0,60}(?:add|create)/.test(c))return false;return /(?:^|[:;,—-]\s*)add (?:\{|one mana|two mana|three mana|four mana|x mana)|\byou (?:add|create) [^.]{0,100}treasure|\bcreate [^.]{0,100}treasure token/.test(c)})}
