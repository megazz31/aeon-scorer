...
export function buildCrossTableRepairs(match={},limit=5){
  ...
  return {modelVersion:'pod-repair-v2',beforeTotal,repairs:repairs.slice(0,Math.max(1,Number(limit)||5)),confidence:{objective:'advanced-pod-match',productCalibration:'experimental'},notes:['Suggestions are true two-way swaps, so every player remains assigned exactly once.','V2 optimizes the current Aeon Match objective only; it does not claim a globally optimal social outcome.']}
}
