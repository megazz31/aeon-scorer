...
export function buildClassThreatAnswerTimeline(results=[]){
  ...
  const timingModels=[...new Set(decks.map(r=>r?.answerProfile?.modelVersion).filter(Boolean))]
  return {modelVersion:'threat-answer-v3',decks:decks.map((r,index)=>({index,turns:turns.map(turn=>{
  ...
  })})),confidence:{productCalibration:'experimental',answerCombination:'multi-opponent-coverage-proxy',answerTimingModels:timingModels},notes:['Threats are matched to relevant semantic answer classes rather than generic interaction.','Per opponent, Aeon uses the strongest relevant answer class; table coverage remains an experimental combination proxy.','V3 consumes class-specific Answer Profile timing when available; historical supplied profiles remain supported but keep their original timing limitations.']}
}
