  ...
  return <main>
    ...
    {result&&<>
      ...
      {view==='summary'&&<>
        ...
        <section className="summaryStrip panel">
          <div><b>Repères de calibration</b><small>repères statistiques, jamais des brackets ni des seuils</small></div><span>Précons <strong>{CALIBRATION.precon}</strong></span><span>cEDH <strong>{CALIBRATION.cedh}</strong></span>
        </section>
      ...
      {view==='diagnostic'&&<>
        <section className="panel diagOverview">
          <h2>Fiabilité et dépendance</h2>
          <div className="grid4"><Stat label="Dépendance commandant" value={`+${result.profile.commanderDelta}`}/><Stat label="Accès commandant médian" value={result.simulation.commanderMedianTurn?`T${result.simulation.commanderMedianTurn}`:'n/d'}/><Stat label="Package opérationnel médian" value={result.simulation.engineMedianTurn?`T${result.simulation.engineMedianTurn}`:'n/d'}/><Stat label="Couverture des données" value={result.profile.coverage} suffix="%" sub="couverture, pas exactitude"/></div>
          {commander&&<p className="note">Après le checkpoint de disruption T4, une option de reprise est accessible à T5 dans <b>{result.simulation.recoveryAfterDisruption}%</b> des séquences. Cette mesure n’est pas une simulation complète d’un wipe.</p>}
        </section>

        <section className="cols">
          <div className="panel"><h2>Pourquoi ce score ?</h2>{Object.entries(result.dimensions).map(([k,v])=><Bar key={k} label={{speed:'Vitesse',consistency:'Consistance',explosiveness:'Explosivité',synergy:'Synergie',interaction:'Interaction accessible',resilience:'Options de reprise'}[k]} value={v}/>)}</div>
