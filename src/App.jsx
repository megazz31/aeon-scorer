...
const RELIABILITY_REASON_LABELS={
  en:{
    'commander-enchantment-animation-combat-not-sequence-simulated':'commander-driven enchantment combat is only partially sequenced',
    'go-wide-combat-damage-not-sequence-simulated':'go-wide combat damage is not simulated turn by turn',
    'donation-goad-opponent-behavior-not-sequence-simulated':'donation/goad depends on opponent behavior Aeon does not simulate',
    'donation-value-not-sequence-simulated':'donation value depends on choices Aeon does not fully sequence',
    'top-library-restricted-cast-not-sequence-simulated':'restricted top-library casting is recognized but not fully sequenced',
    'exact-one-life-loss-frequency-conservative':'exact-one-life triggers are intentionally conservative',
    'activated-ability-mana-and-exhaust-compression-not-sequence-simulated':'ability-only mana and Exhaust compression are not fully sequenced',
    'equipment-attachment-activation-combat-not-sequence-simulated':'Equipment attachment, activation and combat remain conservative',
    'target-cost-reduction-x-value-conservative':'X values with target-based cost reduction remain conservative',
    'known-combo-execution-partial':'known combo execution/timing is only partially modeled',
    'unknown-mana-production':'some lands have uncertain colored mana production',
    'commander-missing':'commander access and dependency are not modeled'
  },
  fr:{
    'commander-enchantment-animation-combat-not-sequence-simulated':'le combat piloté par les enchantements du commandant n’est que partiellement séquencé',
    'go-wide-combat-damage-not-sequence-simulated':'les dégâts de combat go-wide ne sont pas simulés tour par tour',
    'donation-goad-opponent-behavior-not-sequence-simulated':'donation/goad dépend du comportement adverse qu’Aeon ne simule pas',
    'donation-value-not-sequence-simulated':'la valeur de donation dépend de choix qu’Aeon ne séquence pas entièrement',
    'top-library-restricted-cast-not-sequence-simulated':'le cast restreint depuis le dessus est reconnu mais pas entièrement séquencé',
    'exact-one-life-loss-frequency-conservative':'la fréquence des triggers de perte exacte de 1 reste conservatrice',
    'activated-ability-mana-and-exhaust-compression-not-sequence-simulated':'le mana réservé aux capacités et la compression Exhaust ne sont pas entièrement séquencés',
    'equipment-attachment-activation-combat-not-sequence-simulated':'attache, activation et combat Équipement restent conservateurs',
    'target-cost-reduction-x-value-conservative':'les valeurs X avec réduction liée aux cibles restent conservatrices',
    'known-combo-execution-partial':'l’exécution et le timing des combos connues ne sont que partiellement modélisés',
    'unknown-mana-production':'la production colorée de certains terrains reste incertaine',
    'commander-missing':'l’accès et la dépendance au commandant ne sont pas modélisés'
  }
}
function reliabilityLevelLabel(level,lang){return ({high:t(lang,'high','élevée'),moderate:t(lang,'moderate','modérée'),low:t(lang,'low','faible')})[level]||level}
function reliabilityDirectionLabel(direction,lang){
  if(direction==='underestimate')return t(lang,'possible underestimation','sous-estimation possible')
  if(direction==='overestimate')return t(lang,'possible overestimation','surestimation possible')
  if(direction==='uncertain')return t(lang,'direction uncertain','sens de l’écart incertain')
  return t(lang,'no deck-specific bias detected','aucun biais spécifique détecté')
}
function reliabilityReasonLabel(reason,lang){return RELIABILITY_REASON_LABELS[lang]?.[reason?.code]||String(reason?.code||'').replace(/-/g,' ')}
