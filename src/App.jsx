...
export default function App(){
  ...
  return <main>
    <header><div><div className="eyebrow">AEON SCORER v3.1 · SEMANTIC HARDENING</div><h1>Mesurer les sorties d’un deck, avec des preuves lisibles.</h1></div><div className="headerSide"><p>Commander : accès probabiliste, packages, fenêtres de mana, dispersion et dépendance au commandant.</p><span className="calBadge">validation v3.1 en cours</span></div></header>
    ...
    {result&&<>
      <section className="heroScore panel">
        ...
        {commander&&<div className="cmdDelta">Dépendance estimée à <b>{commander.name}</b> : <strong>+{result.profile.commanderDelta}</strong> · accès commandant médian <strong>{result.simulation.commanderMedianTurn?`T${result.simulation.commanderMedianTurn}`:'n/d'}</strong> · package opérationnel médian <strong>{result.simulation.engineMedianTurn?`T${result.simulation.engineMedianTurn}`:'n/d'}</strong> · options de reprise accessibles T5 après disruption T4 <strong>{result.simulation.recoveryAfterDisruption}%</strong>.</div>}
      </section>
      ...
      <section className="cols">
        <div className="panel"><h2>Dimensions</h2>{Object.entries(result.dimensions).map(([k,v])=><Bar key={k} label={{speed:'Vitesse',consistency:'Consistance',explosiveness:'Explosivité',synergy:'Synergie',interaction:'Interaction accessible',resilience:'Options de reprise'}[k]} value={v}/>)}</div>
