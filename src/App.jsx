...
export default function App(){
  ...
  async function analyze(){
    ...
    try{
      ...
      if(total<90||total>105)throw new Error(`La liste contient ${total} cartes. Pour un score Commander fiable, importe la liste complète.`)
      ...
      if(commanderName.trim()){
        setStatus('Chargement du commandant…');cmd=await fetchCard(commanderName)
  ...
  async function importAeon(ev){
    ...
    const map=parseAeonShiftCsv(await f.text());setAeonMap(map);if(!map.size)setError('CSV AeonShift non reconnu ou vide.')
  }
  ...
  return <main>
    ...
    <section className="panel setup">
      <div className="field grow"><label>Decklist</label><textarea value={deckText} onChange={e=>setDeckText(e.target.value)} placeholder={SAMPLE}/><small>{total} carte(s) reconnue(s)</small></div>
      <div className="side">
