-- P7 Aeon Reality: observational game-quality feedback only.
...
begin
  ...
  insert into public.game_observations(user_id,pod_fingerprint,pod_model_version,engine_versions,semantic_versions,turn_band,win_type,balance,dominant_event)
  values(auth.uid(),p_pod_fingerprint,trim(p_pod_model_version),coalesce(p_engine_versions,'{}'),coalesce(p_semantic_versions,'{}'),p_turn_band,p_win_type,p_balance,coalesce(p_dominant_event,'none'))
  returning id into out_id;
  return out_id;
end $$;

revoke all on function public.aeon_submit_game_observation(text,text,text[],text[],text,text,text,text) from public;
grant execute on function public.aeon_submit_game_observation(text,text,text[],text[],text,text,text,text) to anon,authenticated;
