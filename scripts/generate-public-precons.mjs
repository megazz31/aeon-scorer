...
function normalizeScryfall(d){
  ...
  return {
    id:d.id,oracleId:d.oracle_id||d.id,name:d.name,aliases:faces.map(f=>f.name).filter(Boolean),manaCost:d.mana_cost||faces.map(f=>f.mana_cost||'').filter(Boolean).join(' // '),cmc:Number(d.cmc||0),
