...
function evidence(cards){const uniq=new Map();for(const c of cards){const key=c.oracleId||c.id||c.name.toLowerCase();if(uniq.has(key))continue;uniq.set(key,{oracle_id:c.oracleId||c.id,scryfall_id:c.id||null,name:c.name,oracle_text:c.oracle||'',type_line:c.type||'',engine_tags:tagsFor(c)})}return [...uniq.values()]}
...
async function main(){
  ...
  for(const d of detailRows){
    ...
    const all=[...deckCards,commander],freshSnapshot=snapshotHash(all)
    ...
    const profile=d.result?.profile||{},runPayload={user_id:null,deck_id:null,deck_hash:d.deckHash,deck_name:d.name,commander_name:d.commanderName,decklist:d.decklist,cards:evidence(all),result:d.result,engine_version:a.engineVersion,semantic_version:a.semanticVersion,oracle_snapshot_hash:a.oracleSnapshotHash,scryfall_oracle_date:a.scryfallOracleDate,source:'precon',iterations:a.iterations,median:a.median,p20:a.p20,p80:a.p80,peak:a.peak,coverage:a.coverage}
