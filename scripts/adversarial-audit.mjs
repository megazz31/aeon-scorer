...
const pages=await fs.readFile('src/sitePages.jsx','utf8')
const index=await fs.readFile('index.html','utf8')
const readability=await fs.readFile('src/readability.css','utf8')
...
assert(/Package opérationnel/.test(app)&&/Operational package/.test(app))
assert(/option de reprise accessible/i.test(app)&&/accessible recovery option/i.test(app)&&!/récupération après disruption T4→T5/i.test(app),'Recovery UI must describe access options, not claim a simulated rebuild')
assert(/Partner \/ Background/.test(app),'Unsupported dual-command configurations must be disclosed')
assert(/Identité couleur incompatible/i.test(app)&&/Color identity mismatch/i.test(app),'Color identity errors must be localized')
assert(/non exhaustive/i.test(app)&&/non-exhaustive/i.test(app),'Combo UI must disclose incomplete combo coverage in both languages')
assert(/Median power/.test(app)&&/Low output/.test(app)&&/High output/.test(app)&&/Peak/.test(app),'English-first primary dashboard must expose median, P20, P80 and peak')
assert(/Puissance médiane/.test(app)&&/Sortie basse/.test(app)&&/Sortie haute/.test(app)&&/Pic/.test(app),'French switch must retain median, P20, P80 and peak')
assert(!/Cohorte perso|Tes decks/i.test(app),'Personal user cohort must not be presented as a public calibration reference')
assert(/coverage<88/.test(app),'Analysis coverage must stay a summary warning rather than a primary score metric')
...
assert(/Commander power is more than a bracket/.test(app),'Hero must distinguish deck-specific power from Commander Brackets')
assert(!/ne tient pas dans un [«\"]7[»\"]/.test(app),'Legacy “power level 7” hero copy must not return')
assert(/aeon-lang/.test(app)&&/==='fr'\?'fr':'en'/.test(app)&&/languageSwitch/.test(app),'English must be the first-visit default with a persistent EN/FR switch')
assert(/SITE_META\[lang\]/.test(app)&&/document\.documentElement\.lang=lang/.test(app),'SEO metadata and document language must follow the selected language')
assert(/Why Aeon Scorer/.test(pages)&&/Methodology/.test(pages)&&/About/.test(pages),'English public pages must explain purpose, methodology and ownership')
assert(/Pourquoi Aeon Scorer/.test(pages)&&/Méthodologie/.test(pages)&&/À propos/.test(pages),'French public pages must remain available')
assert(/<html lang="en">/.test(index),'Static document language must default to English')
assert(/canonical/.test(index)&&/og:title/.test(index)&&/Commander Deck Power Level Analyzer/.test(index),'Index must retain searchable/social metadata')
assert(/readability\.css/.test(main),'Readability overrides must load after the visual system')
const explicitFontSizes=[...readability.matchAll(/font-size\s*:\s*(\d+(?:\.\d+)?)px/g)].map(m=>Number(m[1]))
assert(explicitFontSizes.length>20&&Math.min(...explicitFontSizes)>=11,'Readability layer must never declare visible text below 11px')
for(const required of [/brandText small\{font-size:12px/,/heroProof\{font-size:14px/,/fieldHeader label,.field label\{font-size:14px/,/field small,.muted,.note[^}]*font-size:13px/,/footerLinks a\{font-size:12px/])assert(required.test(readability),'Critical microcopy must be explicitly enlarged')
assert(/aeon-scorer\.vercel\.app\/pourquoi/.test(sitemap)&&/aeon-scorer\.vercel\.app\/methodologie/.test(sitemap),'Sitemap must expose explanatory routes')
...
console.log('ADVERSARIAL AUDIT OK — semantics, imports, bilingual English-first UI, readable typography, public documentation, SEO, mana edge cases, Commander mulligans, strict benchmark policy and CI wiring')
