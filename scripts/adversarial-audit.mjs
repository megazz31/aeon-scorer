...
assert(/option de reprise accessible/i.test(app)&&!/récupération après disruption T4→T5/i.test(app),'Recovery UI must describe access options, not claim a simulated rebuild')
...
assert(/Pourquoi Aeon Scorer/.test(pages)&&/Méthodologie/.test(pages)&&/À propos/.test(pages),'Public pages must explain project purpose, methodology and ownership')
...
console.log('ADVERSARIAL AUDIT OK — semantics, imports, score-focused UI, public documentation, SEO, mana edge cases, Commander mulligans, strict benchmark policy and CI wiring')
