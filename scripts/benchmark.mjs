  ...
  for(let i=0;i<attempts;i++){
    try{
      ...
      if(r.status===429||r.status>=500){
        ...
        await sleep(Math.max(retry*1000,450*(i+1)));last=new Error(`${r.status} ${r.statusText} ${url}`);continue
      }
...
function normalizeScryfall(d){
  ...
  const manaCost=d.mana_cost||faces.map(f=>f.mana_cost).filter(Boolean).join(' // ')||''
  const producedMana=d.produced_mana||[...new Set(faces.flatMap(f=>f.produced_mana||[]))]
  return {
    name:d.name,aliases:faces.map(f=>f.name).filter(Boolean),oracle,cmc:Number(d.cmc||0),manaCost,type:d.type_line||'',
    colors:d.colors||[],colorIdentity:d.color_identity||[],producedMana:producedMana||[],legalities:d.legalities||{},
    power:d.power??faces[0]?.power??null,toughness:d.toughness??faces[0]?.toughness??null,edhrecRank:d.edhrec_rank??999999,
  }
}
function indexCard(map,c,requested=null){
  map.set(c.name.toLowerCase(),c)
  for(const a of c.aliases||[])map.set(a.toLowerCase(),c)
  if(requested)map.set(String(requested).toLowerCase(),c)
}
async function scryfallCards(names){
  const uniq=[...new Set(names.filter(Boolean))],map=new Map()
  for(let i=0;i<uniq.length;i+=75){
    const batch=uniq.slice(i,i+75),key='scryfall31_'+simpleHash(batch.join('|')),p=path.join(CACHE,key+'.json')
    let j
    try{j=JSON.parse(await fs.readFile(p,'utf8'))}catch{
      const r=await fetchWithBackoff('https://api.scryfall.com/cards/collection',{method:'POST',headers:{'Content-Type':'application/json','User-Agent':'AeonScorer-Calibration/3.1'},body:JSON.stringify({identifiers:batch.map(name=>({name}))})})
      j=await r.json();await fs.writeFile(p,JSON.stringify(j));await sleep(180)
    }
    for(const d of j.data||[])indexCard(map,normalizeScryfall(d))
    for(const name of batch.filter(n=>!map.has(n.toLowerCase()))){
      let card=null
      try{
        const d=await cachedJson('sf31_exact_'+simpleHash(name),`https://api.scryfall.com/cards/named?exact=${encodeURIComponent(name)}`)
        card=normalizeScryfall(d)
      }catch{}
      if(!card){
        try{
          const d=await cachedJson('sf31_fuzzy_'+simpleHash(name),`https://api.scryfall.com/cards/named?fuzzy=${encodeURIComponent(name)}`)
          const c=normalizeScryfall(d);if(acceptableFuzzy(name,c))card=c
        }catch{}
      }
      if(card)indexCard(map,card,name)
      await sleep(80)
    }
  }
  return map
}
function resolveNamed(names,map){
  const cards=[],missing=[]
  for(const n of names){const c=map.get(String(n).toLowerCase());if(c)cards.push({...c});else missing.push(n)}
...
async function loadPrecons(n){
  ...
  const candidates=evenlyPick(rows,Math.min(rows.length,n*5)),raw=[]
  for(const meta of candidates){
    try{
      ...
      const commanderNames=(d.commander||[]).flatMap(c=>Array(Number(c.count||c.quantity||1)).fill(c.name)).filter(Boolean)
      const mainRows=d.mainBoard||d.mainboard||d.cards||[],names=mainRows.flatMap(c=>Array(Number(c.count||c.quantity||1)).fill(c.name)).filter(Boolean)
  ...
  const map=await scryfallCards(raw.flatMap(x=>[...x.names,x.commanderName])),out=[]
  for(const x of raw){
    const {cards,missing}=resolveNamed(x.names,map),commander=map.get(x.commanderName.toLowerCase())
    if(!commander||missing.length||cards.length!==x.names.length)continue
    out.push({source:'precon',name:x.name,commander:{...commander},cards,meta:{...x.meta,rawCount:x.names.length,resolvedCount:cards.length}})
...
async function loadCedh(n){
  ...
  const candidates=evenlyPick(eligible,Math.min(eligible.length,n*4)),allNames=candidates.flatMap(([,d])=>[...d.mainboard,...d.commanders]),map=await scryfallCards(allNames),out=[]
  for(const [name,d] of candidates){
    const {cards,missing}=resolveNamed(d.mainboard,map),commander=map.get(d.commanders[0].toLowerCase())
    if(!commander||missing.length||cards.length!==d.mainboard.length)continue
    out.push({source:'cedh',name,commander:{...commander},cards,meta:{origin:'cEDH Decklist Database mirror',rawCount:d.mainboard.length,resolvedCount:cards.length}})
    if(out.length>=n)break
  }
  return out
}
...
function parseArchidekt(payload){
  ...
  for(const e of payload?.cards||payload?.cardMap||[]){
    const card=e.card?.oracleCard||e.card?.oracle_card||e.oracleCard||e.oracle_card||e.card||{},name=card.name||e.displayName||e.name;if(!name)continue
    const qty=Math.max(1,Number(e.quantity??e.qty??1)||1),catNames=[]
    for(const c of e.categories||[]){if(typeof c==='string')catNames.push(c);else if(typeof c==='number')catNames.push(catMap.get(c)||'');else catNames.push(c?.name||catMap.get(c?.id)||'')}
    const isCmd=catNames.some(c=>/commander/i.test(c))||e.isCommander===true,included=!catNames.some(c=>/maybeboard|sideboard|considering/i.test(c))
...
async function loadArchidekt(n){
  ...
  for(const row of archRows(listing)){
    if(raw.length>=n*3)break
    const id=archDeckId(row);if(!id)continue
    try{
      const d=await cachedJson('arch_'+id,`https://archidekt.com/api/decks/${id}/`),p=parseArchidekt(d)
      if(p.names.length>=95&&p.names.length<=101&&p.commanders.length===1)raw.push({id,name:d.name||row.name||`Archidekt ${id}`,...p})
    }catch{}
  }
  if(!raw.length)return []
  const map=await scryfallCards(raw.flatMap(x=>[...x.names,...x.commanders])),out=[]
  for(const x of raw){
    const {cards,missing}=resolveNamed(x.names,map),commander=map.get(x.commanders[0].toLowerCase())
    if(!commander||missing.length||cards.length!==x.names.length)continue
    out.push({source:'user',name:x.name,commander:{...commander},cards,meta:{archidektId:x.id,owner:USERNAME,rawCount:x.names.length,resolvedCount:cards.length}})
    if(out.length>=n)break
  }
  return out
}
...
function neutralizeTagged(deck,tag,max=99){
  const featured=featureDeck(deck.cards),killNames=new Set(featured.filter(c=>c.tags.includes(tag)).slice(0,max).map(c=>c.name.toLowerCase()))
  let i=0
  return {...deck,cards:deck.cards.map(c=>killNames.has(c.name.toLowerCase())?{...c,name:`Calibration Blank ${tag} ${++i}`,oracle:'',type:'Calibration',producedMana:[],power:null,toughness:null}:c)}
}
...
async function evaluateQuality(decks,scored){
  const pre=scored.filter(x=>x.source==='precon'),cedh=scored.filter(x=>x.source==='cedh'),usr=scored.filter(x=>x.source==='user'),ps=splitHalf(pre),cs=splitHalf(cedh)
  const sepAll=separation(pre,cedh),sepHold=separation(ps.hold,cs.hold),sepCal=separation(ps.cal,cs.cal)
  const completeness=decks.every(d=>d.meta?.rawCount===d.meta?.resolvedCount)
  ...
  const gates=[
    ...
    {id:'semantic-scale',ok:sepAll.preconMedian>=42&&sepAll.preconMedian<=55&&sepAll.cedhMedian>=72&&sepAll.cedhMedian<=88,detail:`precon ${sepAll.preconMedian.toFixed(1)}, cEDH ${sepAll.cedhMedian.toFixed(1)}`},
  ]
  return {score:gates.filter(g=>g.ok).length,total:gates.length,gates,separation:{all:sepAll,calibration:sepCal,holdout:sepHold},repeatability,fastManaSensitivity,tutorSensitivity,commanderDependency}
}
...
function markdown(report){
  ...
  lines.push('','## Anchors','',`- Precons: ${report.counts.precon}`,`- cEDH: ${report.counts.cedh}`,`- User/public: ${report.counts.user}`,`- Holdout AUC: ${q.separation.holdout.auc.toFixed(3)}`,`- Overall median gap: ${q.separation.all.gap.toFixed(1)}`,'','## Deck results','','| Source | Deck | Commander | Median | P20 | P80 | Peak | Dispersion | Consistency |','|---|---|---|---:|---:|---:|---:|---:|---:|')
...
const scored=[]
for(let i=0;i<decks.length;i++){const d=decks[i];console.log(`[${i+1}/${decks.length}] ${d.source}: ${d.name}`);scored.push(scoreDeck(d))}
const quality=await evaluateQuality(decks,scored)
const report={generatedAt:new Date().toISOString(),model:'sequence-access-v3.1-semantic',iterations:ITERATIONS,counts:{total:decks.length,precon:precons.length,cedh:cedh.length,user:user.length},quality,decks:scored}
